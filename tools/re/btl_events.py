...
근거(DLL, ImageBase 0x10000000):
  ...
  조건 판정    0x10056400 — 발동 횟수(+0x14)가 최대(+0xc, 0=제한 없음. 0xffff 는 부호 있는 비교라 절대 안 터짐)에 닿으면 건너뜀. 조건은 모두 참이어야 한다(AND).
...
COND = {
    ...
    102: "그 캐릭터가 장비를 가졌나 (0x1004f2f0)",
    200: "유닛 사망 / 부대 전멸·생존 (0x1004f360)",
    201: "유닛(부대 전원)이 죽었거나 전장에 없다 — 인자1 은 안 읽는다 (0x1004f550)",
...
def show(game, num, txr):
    ...
    for i, (mx, kind, cs, as_) in enumerate(evs):
        print("  이벤트 %d  최대 발동 %s  갈래 %d" % (i, "안 터짐" if mx == 0xFFFF else mx, kind))
        for c, p in cs:
